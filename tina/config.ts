...
export default defineConfig({
  ...
  clientId:
    ...
    (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_TINA_CLIENT_ID) ||
    "37dc3ba5-1d37-49ce-a046-bcb3ae4a98ba",
  token:
    process.env.TINA_TOKEN ||
    process.env.VITE_TINA_TOKEN ||
    (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_TINA_TOKEN) ||
    "72bebcdc29641394171b225f1de0c76d8614a04c",
    ...
    collections: [
      {
        ...
        ui: {
          ...
          filename: {
            slugify: (values) => {
              return `${values?.order || "00"}-${(values?.title || "")
                .toLowerCase()
                .replace(/[^a-z0-9]/g, "-")}`;
            },
          },
      ...
      },
      {
        ...
        ui: {
          ...
          filename: {
            slugify: (values) => {
              return `${values?.order ? String(values.order).padStart(2, "0") : "01"}-${(values?.slug || values?.title || "")
                .toLowerCase()
                .replace(/[^a-z0-9]/g, "-")}`;
            },
          },
